...
public interface MessageRepository extends PagingAndSortingRepository<Message, Long>, JpaRepository<Message, Long> {
    ...
    @Query( " select m" +
            " from Message as m inner join m.senderUserReg as s inner join s.firstSenderChats as f" +
            " where (f.id = :userRegId) and (m.isLastMessage = true)")
    Page<Message> findByUserRegIdForSimpleUser(@Param("userRegId") Long userRegId, Pageable pageable);

    @Query( " select m" +
            " from Message as m inner join m.senderUserReg as s inner join s.firstReceiverChats as f" +
            " where (f.id = :userRegId) and (m.isLastMessage = true)")
